...
# 4. mkdocs.yml
mkdocs_yml = '''site_name: iOS Traffic Intel
theme: readthedocs
...
nav:
  ...
  - CLI 参考: CLI.md
  - 目录结构: structure.md
  - Go-ios 集成: GOIOS_INTEGRATION.md
...
with open(f"{base_dir}/mkdocs.yml", "w", encoding="utf-8") as f:
    f.write(mkdocs_yml)
...
# 删除旧的 README.md，创建 README.rst
if os.path.exists(f"{base_dir}/README.md"):
    os.remove(f"{base_dir}/README.md")
with open(f"{base_dir}/README.rst", "w", encoding="utf-8") as f:
    f.write(readme_rst)

print("docs/CLI.md, docs/structure.md, mkdocs.yml, README.rst 创建完成")
print("README.md 已删除")
